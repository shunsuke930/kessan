...
- **色分け判定**（`StatusService.gs`）: 案件全体のステータスは決算日基準の3段階判定のみ
  （通常/要注意/遅延、要件定義書3.5）。個別タスクのバー色・対応が必要なタスク一覧の
  バッジは、そのタスク自身の期限による判定（手動上書き`manualOverride`可）。この2つは
  独立しており、個別タスクが期限切れでも案件全体のステータスには影響しない
  （自動生成された初期タスクの期限が、同期直後で未着手のまま経過していることは
  珍しくないため、それだけで案件全体を「遅延」扱いにしないための仕様）。
...
- **担当者フィルター**: ダッシュボード上部の「担当者で絞り込み」で特定の担当者を選ぶと、
  対応が必要なタスク・年間カレンダーがその担当者の案件だけに絞られる
  （サーバーへの再取得はせず、取得済みデータをブラウザ側でフィルタリングするだけなので
  即座に切り替わる）。10名規模のチームで、自分の担当分だけ見たい場合に利用する。
- **「季節内」フィルター**（`isInSeason` in `JavaScript.html`）: 対応が必要なタスク・
  年間カレンダーとも、基準日の月を中心に前後2ヶ月（例: 基準日が8月なら6月〜10月決算）に
  決算日が入っている顧客だけを対象にする。決算がまだ遠い顧客のノイズを削り、
  「今まさに動きがある顧客」だけに絞り込むための共通ルール。
- **対応が必要なタスク**: 未完了かつ「要注意」または「遅延」のタスクを、期限の近い順に
  一覧表示する。「決算準備中の会社」（基準日が決算日以前）と「決算中の会社」（基準日が
  決算日より後）で表を分けている。各行にはチェックボックス（チェックすると即座に
  タスクを「完了」にして一覧から消える）とコメント欄（自由記述、フォーカスを外すと
  自動保存）があり、一覧から離れずに素早く処理できる。タスク名をクリックすると、
  期限や進捗を細かく調整できる編集モーダルが開く。
