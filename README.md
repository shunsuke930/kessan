...
- **色分け判定**（`StatusService.gs`）: 案件全体（サマリー表・案件カードのバッジ）の
  ステータスは決算日基準の3段階判定のみ（通常/要注意/遅延、要件定義書3.5）。
  個別タスクのバー色は、そのタスク自身の期限による判定（手動上書き`manualOverride`可）。
  この2つは独立しており、個別タスクが期限切れでも案件全体のバッジには影響しない
  （自動生成された初期タスクの期限が、同期直後で未着手のまま経過していることは
  珍しくないため、それだけで案件全体を「遅延」表示にしないための仕様）。
...
- **担当者フィルター**: ダッシュボード上部の「担当者で絞り込み」で特定の担当者を選ぶと、
  年間カレンダー・サマリー表・案件カードのすべてがその担当者の案件だけに絞られる
  （サーバーへの再取得はせず、取得済みデータをブラウザ側でフィルタリングするだけなので
  即座に切り替わる）。10名規模のチームで、自分の担当分だけ見たい場合に利用する。
- **対応が必要なタスク**: ダッシュボード最上部（赤枠）に、未完了かつ「要注意」または
  「遅延」のタスクだけを期限の近い順に一覧表示する。案件カードを一つずつ開かなくても、
  今すぐ手を付けるべきものが一目でわかる。担当者フィルターにも連動する。クリックすると
  該当タスクの編集モーダルが直接開く。
