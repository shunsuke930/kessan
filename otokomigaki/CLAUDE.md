...
- **部屋グレードの必要pt**（`ROOM_GRADES`）: Grade1=0 / Grade2=50 / Grade3=225 /
  ...
  間だけ意図的に一致しなくなる。グレード名自体は`Header.tsx`には表示しない
  （最上部のヘッダーは`Lv.`とストリークのみ）。「◯◯まであと△pt」の表示
  （`GradeProgress.tsx`）が唯一のグレード名表示箇所。
...
## 今日のタスク（縦並びリスト、`TaskList.tsx`）

有効なパッケージを縦に積み上げて表示する、素直な縦スクロールのリスト。
横スワイプのカルーセルは廃止した（以前はCSS scroll-snapによる横カルーセル
だったが、縦スクロールのみのシンプルな一覧に変更）。

- パッケージごとに見出し（パッケージ名＋達成数「3/6」）があり、見出し全体を
  タップすると開閉できる。開閉状態は「閉じたパッケージIDの集合
  (`closedPackageIds`)」で管理し、そこに入っていないパッケージは常に開いた
  扱いになる。初期状態は空集合＝全パッケージが開いた状態。新しく解放/ONに
  したパッケージも自動的に開いた状態で表示される。
- ページ全体スクロール（下記セクション参照）の一部としてそのまま積み上がる
  だけで、内部スクロールやページ切り替えは持たない。
...
- **チェック演出**: タスクを1つチェックするたびに`useGameState`の`checkPulse`
  ...
  している。
- **タスク行のチェック演出**（`TaskList.tsx`）: タスクをチェック（未完了→
  完了）した瞬間だけ、その行のIDを`flashingTaskIds`（ローカルstate、
  800ms後に自動でsetTimeoutで消す）に入れ、2つのCSSアニメーションを
  同時に再生する。アンチェック時は再生しない。
  - 行の枠が`.animate-check-glow`（0.4秒）でエメラルド色にふわっと光る。
  - pt表示バッジの上に`.animate-float-pt`（0.8秒）で「+◯pt」が
    ふわっと浮かんで消える（`opacity`と`translateY`のみ、ライブラリ不使用）。
  いずれも`index.css`で`prefers-reduced-motion: reduce`のとき無効化される。
